...
export default function ApexDailyLog() {
  ...
  const canNext = dateStr < todayStr;
  ...
  const goPrev = () => { const d = new Date(dateStr + "T00:00:00"); d.setDate(d.getDate() - 1); setDateStr(fmtDate(d)); };
  const goNext = () => { if (canNext) { const d = new Date(dateStr + "T00:00:00"); d.setDate(d.getDate() + 1); setDateStr(fmtDate(d)); } };
